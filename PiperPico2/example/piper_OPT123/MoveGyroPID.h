...
const float wheelDiameter = 4.1f;                      // cm
const float wheelCircumference = PI * wheelDiameter;
const int pulsesPerRevolution = 250;
...
char ch_lrs;
int motor_slow = 18;
int fw_to_rotate = 180;  // pulse สำหรับถอยออกจากเส้น (ปรับได้)

// PID สำหรับหมุน (ปรับ aggressive ขึ้นนิดสำหรับ BNO085)
float r_kp = 2.8f, r_ki = 0.035f, r_kd = 18.0f;   // หมุนขวา
float l_kp = 2.6f, l_ki = 0.030f, l_kd = 16.5f;   // หมุนซ้าย
...
// ค่าคงที่หลัก
const float TOLERANCE_TURN     = 2.4f;     // องศา สำหรับหมุน
const float TOLERANCE_STRAIGHT = 1.8f;     // องศา สำหรับเดินตรง
const unsigned long TIMEOUT_MS = 1800;     // ms (1.8 วินาที)
...
void turnGyro(int speed, float degree) {
  ...
  if (needForwardBackup || needBackwardBackup) {
    ...
    if (needForwardBackup) {
      ...
      } while (encoder1.Poss_L() > -(fw_to_rotate - 50));

      robot.motor('A', 20); robot.motor('C', 20); delay(20);
      robot.motor('A', 0); robot.motor('C', 0); delay(30);
    }
  ...
  while (true) {
    ...
    if (abs(error) < 2.5f) {
    output *= (abs(error) / 2.5f) * 0.5f;   // ลดลงตาม error
}
    ...
    if (abs(error) <= TOLERANCE_TURN && abs(output) <= 5.0f) {
      stableCount++;
      if (stableCount >= 8) {
        int brakeValue = isRight ? -14 : 14;
        robot.motor('A', brakeValue);
        robot.motor('C', -brakeValue);
        delay(25);
        robot.motor('A', 0);
        robot.motor('C', 0);
        delay(40);
