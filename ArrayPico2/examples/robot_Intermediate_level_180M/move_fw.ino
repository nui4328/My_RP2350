...
void move_fw(int sl, float kp, float dist, String _line) {
  ...
  const int slow_base_speed = 20;   // ความเร็วพื้นฐานในโหมดช้า ๆ ("line")
  const float slow_kp       = 1.75f; // PID gain สำหรับช่วงช้า (sensitive กว่า)
...
if (_line == "line") {
    ...
    const int   max_speed       = 30;       // จำกัดสูงสุดไม่ให้เกิน 9 (ช้าจริง)
    ...
    while (true) {  // วนจนกว่าจะเจอเส้น ไม่ต้อง timeout เพราะระยะใกล้อยู่แล้ว
        ...
        integral += angleError * dt;
        integral = constrain(integral, -30, 30);  // windup น้อยลงเพราะช้า
