...
const float my_GYRO1601::GYRO_THRESHOLD = 0.3; // Reduced threshold for stricter deadzone
const float my_GYRO1601::ALPHA = 0.95;        // Reduced ALPHA to give more weight to accelerometer
const float my_GYRO1601::ACCEL_FILTER_ALPHA = 0.1; // For accelerometer filtering

unsigned long my_GYRO1601::_lastTime = 0;
float my_GYRO1601::_angleX = 0.0;
float my_GYRO1601::_angleY = 0.0;
float my_GYRO1601::_angleZ = 0.0;
float my_GYRO1601::_gyroOffsetX = 0.0;
float my_GYRO1601::_gyroOffsetY = 0.0;
float my_GYRO1601::_gyroOffsetZ = 0.0;
float my_GYRO1601::_accelX_prev = 0.0;
float my_GYRO1601::_accelY_prev = 0.0;
float my_GYRO1601::_accelZ_prev = 0.0;
...
bool my_GYRO1601::begin() {
  ...
  Wire1.write(my_GYRO1601_CHIP_ID);
  ...
  // Reset sensor
  writeRegister(my_GYRO1601_CMD, 0xB6); // Soft reset
  delay(100);

  // Configure accelerometer (±2g, ODR 400Hz)
  writeRegister(my_GYRO1601_ACCEL_CONF, 0x2A); // ODR = 400Hz
  writeRegister(my_GYRO1601_ACCEL_RANGE, 0x03); // ±2g

  // Configure gyroscope (±2000 dps, ODR 400Hz)
  writeRegister(my_GYRO1601_GYRO_CONF, 0x2A); // ODR = 400Hz
  writeRegister(my_GYRO1601_GYRO_RANGE, 0x00); // ±2000 dps

  // Start accelerometer and gyroscope
  writeRegister(my_GYRO1601_CMD, 0x11); // Start accelerometer
  delay(50);
  writeRegister(my_GYRO1601_CMD, 0x15); // Start gyroscope
  delay(50);
...
// Reset yaw to zero
void my_GYRO1601::resetYaw() {
  _angleZ = 0.0;
}

// Reset all angles (roll, pitch, yaw) to zero
void my_GYRO1601::resetAngles() {
  _angleX = 0.0;
  _angleY = 0.0;
  _angleZ = 0.0;
}
...
void my_GYRO1601::readAccelGyro(int16_t *ax, int16_t *ay, int16_t *az, int16_t *gx, int16_t *gy, int16_t *gz) {
  Wire1.beginTransmission(_address);
  Wire1.write(my_GYRO1601_GYRO_DATA);
  Wire1.endTransmission();
...
bool my_GYRO1601::calibrateGyro() {
  const int CALIBRATION_SAMPLES = 500; // Increased sample count
