...
const int pulsesPerRevolution = 18;                  // จำนวนพัลส์ต่อรอบ (ปรับใหม่ตามจริง)
...
void turnGyro(int speed, float degree) {
  ...
  const int  BACKUP_SPEED    = 50;     // ความเร็วถอย (20-35 ดีที่สุด)
  ...
  while (true) {
    ...
    if (abs(error) <= TOLERANCE && abs(output) <= 2.0f) {
      stableCount++;
