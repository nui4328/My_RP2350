...
void fline(int spl, int spr, float kp, float distance, char nfc, char splr, int power, String sensor, int endt)
{
    ...
    int target_pwm = min(spl, spr);               // PWM สูงสุด (base speed)
    float max_speed = (float)target_pwm;          // สมมติ PWM ≈ cm/s (ปรับตามจริงได้)

    // รีเซ็ต encoder
    encoder.resetEncoders();
    long initial_ticks = encoder.Poss_L();

    float current_speed = 0.0f;                   // ความเร็วปัจจุบัน (cm/s)
    unsigned long prevT = millis();

    float I = 0.0f;                               // reset integral ถ้าต้องการ
    ...
    if(spl<17)
      {
        SLOW_DOWN_START_CM   = 5.0;     // เริ่มชะลอเมื่อเหลือระยะนี้ (cm)
         MIN_SPEED           = 10.0;     // ความเร็วต่ำสุด (ป้องกันมอเตอร์หยุดนิ่ง)
      }
    if(slmotor > spl)
      {
        slmotor = spl/1.2;            
    ...
    while (true)
    {
        ...
        // อ่าน encoder และคำนวณระยะทาง
        long current_ticks = encoder.Poss_L();
        long delta_ticks = current_ticks - initial_ticks;
        float traveled_cm = delta_ticks * CM_PER_TICK;
        float remaining_cm = distance - traveled_cm;

        if (remaining_cm <= 0 || distance <= 0) {
            break;
        }

        // ── คำนวณความเร็วเป้าหมาย ── แบบเร่งแล้วคงที่ ─────────────────
    float target_speed;

    if (ch_p == 1) {
        // ออกตัวเร็วทันที ไม่ต้อง ramp ช้า
        target_speed = max_speed;
        current_speed = max_speed;          // เริ่มด้วยความเร็วสูงสุดเลย
    }
    else {
        // โหมดปกติ: เร่งขึ้นทีละน้อย
        ...
        // ── คำนวณ error จากเซ็นเซอร์เส้น ──────────────────────────
        if (kp <= 0.45) {
            if (read_sensorA(2) > md_sensorA(2) && read_sensorA(3) > md_sensorA(3) &&
                read_sensorA(4) > md_sensorA(4) && read_sensorA(5) > md_sensorA(5)) {
                errors = 0;
            }
            else if (read_sensorA(5) < md_sensorA(5) && read_sensorA(6) < md_sensorA(6) &&
                     read_sensorA(7) < md_sensorA(7)) {
                errors = 10;
            }
            else if (read_sensorA(2) < md_sensorA(2) && read_sensorA(1) < md_sensorA(1) &&
                     read_sensorA(0) < md_sensorA(0)) {
                errors = -10;
            }
            else {
                errors = error_AA();
        ...
        I += errors * dt;  // ใช้ dt จริงแทน 0.00005
        float D = errors - previous_error;
        previous_error = errors;

        float PID_output = (kp * P) + (0.0001f * I) + (kd_f * D);

        // ── ส่ง PWM ─────────────────────────────────────────────────
        int base_pwm = (int)current_speed;
        int left_pwm  = constrain(base_pwm - PID_output, -100, 100);
        int right_pwm = constrain(base_pwm + PID_output, -100, 100);
        updateBattery();
...
P = errors;
                ...
                delayMicroseconds(50);
                if (analogRead(26) <  (sensorMinC[0]+md_sensorC(0))/2 || analogRead(27) < (sensorMinC[1]+md_sensorC(1))/2) {
                    break;
                }
              }
            if (endt > 0) 
    ...
    while (true)
{
    unsigned long now = millis();
    float dt = (now - prevT) / 1000.0f;
    if (dt < 0.001f) dt = 0.001f;           // ป้องกัน dt เกินจริง
    prevT = now;

    // ── อ่าน encoder + คำนวณระยะ ───────────────────────────────
    long current_ticks = encoder.Poss_L();
    long delta_ticks = current_ticks - initial_ticks;
    float traveled_cm = delta_ticks * CM_PER_TICK;
    //float remaining_cm = distance - traveled_cm;   // อาจไม่ได้ใช้แล้วก็ได้

    // ── ตรวจจบการวิ่ง (เซ็นเซอร์เส้น) ───────────────────────────
    ...
    if (should_stop) {
        Motor(0, 0);           // หยุดทันที หรือจะค่อย ๆ ลดก็ได้
        break;
    }
    ...
    // ── ส่ง PWM ──────────────────────────────────────────────────
    int base_pwm = (int)current_speed;   // หรือ round() ก็ได้
    int left_pwm  = constrain(base_pwm - PID_output, -100, 100);
    int right_pwm = constrain(base_pwm + PID_output, -100, 100);
...
void bline(int spl, int spr, float kp, float distance, char nfc, char splr, int power, String sensor, int endt) {
    ...
    // รีเซ็ต encoder
    encoder.resetEncoders();
    long initial_ticks = abs(encoder.Poss_L());
    ...
    while (true)
    {
        ...
        // อ่าน encoder และคำนวณระยะทาง
        long current_ticks = abs(encoder.Poss_L());
        long delta_ticks = current_ticks - initial_ticks;
        float traveled_cm = delta_ticks * CM_PER_TICK;
        float remaining_cm = distance - traveled_cm;
    ...
    if (ch_p == 1) {
        ...
        if (kp <= 0.45) {
            ...
            else if (read_sensorB(5) < md_sensorB(5) && read_sensorB(6) < md_sensorB(6) &&
                     read_sensorB(7) < md_sensorB(7)) {
                errors = 10;
            }
            else if (read_sensorB(2) < md_sensorB(2) && read_sensorB(1) < md_sensorB(1) &&
                     read_sensorB(0) < md_sensorB(0)) {
                errors = -10;
            }
            else {
                errors = error_BB();
...
bool gyroOK(float stable_threshold = 2.0, int samples = 15)
{
    ...
    {
        float g = my.gyro('z');
        if (isnan(g)) return false;  // Gyro เสีย
        sum += abs(g - prev);
        prev = g;
        delay(4);
    }
...
void place_left_in(int speed, int degree, int offset)
{
    ...
    if (useGyro && my.gyro('z') < 200 && my.gyro('z') > -200)
    {
        ...
        while (true)
        {
            ...
            if (millis() - start > timeout) break;  // กันค้าง
        }
  ...
  while (1) 
      {
        ...
        if(sensorss == "a0")
          ...
          {
             if(analogRead(26)  < (sensorMinC[0]+sensorMaxC[0])/2)
              {
                break;
              }
          }
          ...
          {
             if(analogRead(27)  < (sensorMinC[1]+sensorMaxC[1])/2)
              {
                break;
              }
          }
  ...
  if(sp == 'p')
    {
      if(sensorss == "a0")
          ...
          {
            do{Motor(spl, spr);delayMicroseconds(50);}while(analogRead(26)  < (sensorMinC[0]+sensorMaxC[0])/2);
          }
        else if(sensorss == "c1")
          {
             do{Motor(spl, spr);delayMicroseconds(50);}while(analogRead(27)  < (sensorMinC[0]+sensorMaxC[1])/2);
          }
      }
    else{}
    // soft stop
