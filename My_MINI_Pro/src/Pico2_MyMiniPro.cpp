...
constexpr uint16_t kCalibrationProgressDurationMs = 25;
...
const MyMiniPro::Settings kPico2Settings = {
    3, 0x50, 0x68, 1000, 4095, 3, 5000, true};
...
void MyMiniPro::Motor(int left, int right) {
  driveMotor(leftMotorPins_, left);
  driveMotor(rightMotorPins_, right);
}

void MyMiniPro::stopMotors() { Motor(0, 0); }

void MyMiniPro::resetForwardLineFollowerState() {
  forwardLineDetected_ = false;
  forwardLinePreviousErrorValid_ = false;
  forwardLineError_ = 0.0f;
  forwardLinePreviousError_ = 0.0f;
  forwardLineIntegral_ = 0.0f;
  lastForwardLineControlAt_ = 0;
}

void MyMiniPro::configureForwardLineFollower(
    const ForwardLineFollowerSettings &settings) {
  forwardLineFollowerSettings_ = settings;
  forwardLineFollowerSettings_.baseSpeed =
      constrain(forwardLineFollowerSettings_.baseSpeed, 0, 100);
  // Preserve the original serviceForwardLineFollower() behaviour when the
  // compact FLine() interface is not being used.
  forwardLineLeftBaseSpeed_ = forwardLineFollowerSettings_.baseSpeed;
  forwardLineRightBaseSpeed_ = forwardLineFollowerSettings_.baseSpeed;
  forwardLineFollowerSettings_.maxSteering =
      constrain(forwardLineFollowerSettings_.maxSteering, 0, 100);
  if (forwardLineFollowerSettings_.integralLimit < 0.0f) {
    forwardLineFollowerSettings_.integralLimit =
        -forwardLineFollowerSettings_.integralLimit;
  }
  if (forwardLineFollowerSettings_.controlIntervalUs < 1000) {
    forwardLineFollowerSettings_.controlIntervalUs = 1000;
  }
  forwardLineFollowerSettings_.steeringSign =
      forwardLineFollowerSettings_.steeringSign < 0 ? -1 : 1;
  resetForwardLineFollowerState();
}

void MyMiniPro::setForwardLineFollowerEnabled(bool enabled) {
  forwardLineFollowerEnabled_ = enabled;
  resetForwardLineFollowerState();
  if (!enabled) stopMotors();
}

bool MyMiniPro::forwardLineFollowerEnabled() const {
  return forwardLineFollowerEnabled_;
}

bool MyMiniPro::forwardLineDetected() const { return forwardLineDetected_; }

float MyMiniPro::forwardLineError() const { return forwardLineError_; }

void MyMiniPro::setFLinePID(float kd, float ki) {
  forwardLineFollowerSettings_.kd = kd;
  forwardLineFollowerSettings_.ki = ki;
}

void MyMiniPro::FLine(int leftSpeed, int rightSpeed, float kp) {
  forwardLineLeftBaseSpeed_ = constrain(leftSpeed, 0, 100);
  forwardLineRightBaseSpeed_ = constrain(rightSpeed, 0, 100);
  forwardLineFollowerSettings_.kp = kp;

  // Make the compact call ready to use without a separate enable call.
  if (!forwardLineFollowerEnabled_) {
    setForwardLineFollowerEnabled(true);
  }
  serviceForwardLineFollower();
}

bool MyMiniPro::serviceForwardLineFollower() {
  if (!forwardLineFollowerEnabled_) return false;
  if (!hasCalibration(Array::Front)) {
    resetForwardLineFollowerState();
    stopMotors();
    return false;
  }
  ...
  if (lastForwardLineControlAt_ != 0 &&
      now - lastForwardLineControlAt_ <
          forwardLineFollowerSettings_.controlIntervalUs) {
    return false;
  }

  const float dt = lastForwardLineControlAt_ == 0
                       ? forwardLineFollowerSettings_.controlIntervalUs / 1000000.0f
                       : (now - lastForwardLineControlAt_) / 1000000.0f;
  lastForwardLineControlAt_ = now;
  scanFront();

  float darknessSum = 0.0f;
  float weightedPosition = 0.0f;
  for (uint8_t channel = 0; channel < kSensorCount; ++channel) {
    // Calibration maps white to high values and black to low values.
    const float darkness = 1000.0f - normalized(Array::Front, channel);
    darknessSum += darkness;
    weightedPosition += darkness * channel;
  }

  if (darknessSum < forwardLineFollowerSettings_.minimumDarknessSum) {
    // A deliberate stop is safer than searching with an unverified direction.
    forwardLineDetected_ = false;
    forwardLinePreviousErrorValid_ = false;
    forwardLineError_ = 0.0f;
    forwardLineIntegral_ = 0.0f;
  ...
  constexpr float kSensorCenter = (kSensorCount - 1) / 2.0f;
  forwardLineError_ = weightedPosition / darknessSum - kSensorCenter;
  if (forwardLinePreviousErrorValid_) {
    forwardLineIntegral_ += forwardLineError_ * dt;
    forwardLineIntegral_ = constrain(forwardLineIntegral_,
                                     -forwardLineFollowerSettings_.integralLimit,
                                     forwardLineFollowerSettings_.integralLimit);
  } else {
    forwardLineIntegral_ = 0.0f;
  ...
  const float derivative = forwardLinePreviousErrorValid_
                               ? (forwardLineError_ - forwardLinePreviousError_) / dt
                               : 0.0f;
  forwardLinePreviousError_ = forwardLineError_;
  forwardLinePreviousErrorValid_ = true;
  forwardLineDetected_ = true;

  const float correction = forwardLineFollowerSettings_.steeringSign *
                           (forwardLineFollowerSettings_.kp * forwardLineError_ +
                            forwardLineFollowerSettings_.ki * forwardLineIntegral_ +
                            forwardLineFollowerSettings_.kd * derivative);
  const int steering = constrain(static_cast<int>(correction),
                                 -forwardLineFollowerSettings_.maxSteering,
                                 forwardLineFollowerSettings_.maxSteering);
  const int leftBaseSpeed = compensateBaseMotorSpeed(forwardLineLeftBaseSpeed_);
  const int rightBaseSpeed =
      compensateBaseMotorSpeed(forwardLineRightBaseSpeed_);
  Motor(leftBaseSpeed + steering, rightBaseSpeed - steering);
  return true;
}
...
void MyMiniPro::scanMux(const MuxPins &mux,
  ...
  for (uint8_t channel = 0; channel < kSensorCount; ++channel) {
    readings[channel] = readChannel(mux, channel);
  }
}
...
void MyMiniPro::scanFront() { scanMux(frontMux_, frontCurrent_); }

void MyMiniPro::scanRear() { scanMux(rearMux_, rearCurrent_); }
...
uint16_t MyMiniPro::normalized(Array array, uint8_t channel) const {
  ...
  if (sensor.maximum <= sensor.minimum) return 0;

  long mapped = map(sensor.current, sensor.minimum, sensor.maximum, 0, 1000);
  return static_cast<uint16_t>(constrain(mapped, 0L, 1000L));
}
...
void MyMiniPro::updateCalibration() {
  ...
  // Short periodic beeps while the robot is being moved over white and black.
