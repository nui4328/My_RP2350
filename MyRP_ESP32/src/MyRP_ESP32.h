...
public:
    ...
    const float SPEED_FACTOR_CM_S = 3.85f;
    float SPEED_FACTOR_CM_Sfw = 3.85;
    void sp_factor_cm_s_forFW(float SP_FACTOR_CM_SS);
    float SPEED_FACTOR_CM_Sbw = 1.85;
...
private:
    ...
    float batteryScale = 1.0f;           // เก็บ scale ล่าสุด
    unsigned long lastBatteryUpdate = 0; // ใช้ควบคุมเวลาอัพเดท
