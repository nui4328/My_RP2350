// ----------------------- my_GYRO1600.cpp (เวอร์ชันแก้ error + warning สนิท) -----------------------
#include "my_GYRO1600.h"
#include <math.h>  // สำหรับ sqrt(), abs()

// Static variable definitions
const float my_GYRO1600::GYRO_THRESHOLD = 0.3; // Reduced threshold for stricter deadzone
const float my_GYRO1600::ALPHA = 0.95;        // Reduced ALPHA to give more weight to accelerometer
const float my_GYRO1600::ACCEL_FILTER_ALPHA = 0.1; // For accelerometer filtering
...
bool my_GYRO1600::begin() {
  ...
  // Reset sensor
  writeRegister(my_GYRO1600_CMD, 0xB6); // Soft reset
  delay(100);

  // Configure accelerometer (±2g, ODR 400Hz)
  writeRegister(my_GYRO1600_ACCEL_CONF, 0x2A); // ODR = 400Hz
  ...
  writeRegister(my_GYRO1600_CMD, 0x11); // Start accelerometer
  delay(50);
  writeRegister(my_GYRO1600_CMD, 0x15); // Start gyroscope
  delay(50);
...
// Reset yaw to zero
void my_GYRO1600::resetYaw() {
  _angleZ = 0.0;
}

// Reset all angles (roll, pitch, yaw) to zero
void my_GYRO1600::resetAngles() {
  _angleX = 0.0;
  _angleY = 0.0;
  _angleZ = 0.0;
}
...
bool my_GYRO1600::calibrateGyro() {
  const int CALIBRATION_SAMPLES = 800; // Increased sample count
...
void my_GYRO1600::reCalibrateGyro() {
  const int CALIBRATION_SAMPLES = 100; // Increased sample count
  float sumX = 0.0, sumY = 0.0, sumZ = 0.0;
  float sumAX = 0.0, sumAY = 0.0, sumAZ = 0.0;
  float varX = 0.0, varY = 0.0, varZ = 0.0;
  int16_t gx, gy, gz;
