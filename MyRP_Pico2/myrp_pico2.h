...
float error_setpoint(float _setpoint)
    {
          present_position = position_A() / ((numSensor - 1) * 10) ;
          setpoint = _positions;
          errors = setpoint - present_position;
...
void sensor_position( float positionss)
    {
        _positions = positionss;
        setpoint_fw = true;
    }
// ค่าคงที่สำหรับ fline() 
// ================================================
const float CRUISE_PWM_VAL    = 90.0f;
const float MIN_PWM_VAL       = 28.0f;
const float ACCEL_RATE_PWM_S  = 140.0f;
const float DECEL_RATE_PWM_S  = 200.0f;
const float SPEED_FACTOR_CM_S = 2.45f;
// ตัวแปร global สำหรับเก็บสถานะ 'p' (ข้ามเส้น)
bool crossedLine = false;

// ================================================
// ฟังก์ชัน fline เวอร์ชันพร้อมแข่ง
// ================================================
void fline(int spl, int spr, float kp, String target, char nfc, char splr, int power, String sensor, int endt)
{
    ...
    // =============== วิ่งตรง (รองรับทั้งระยะทางและโหมดตรวจจับ) ===============
    {
        ...
        float accel_time = (cruise - minp) / ACCEL_RATE_PWM_S;
        float decel_time = (cruise - minp) / DECEL_RATE_PWM_S;

        // ถ้าเคยข้ามเส้นมา (splr == 'p' จากคำสั่งก่อนหน้า) → ไม่เร่ง
        ...
        while (true)
        {
            ...
            float PID = kp * errors + 0.0001f * I + current_Kd * (errors - prevE);
            prevE = errors;
            ...
            if (isSpecialMode) {
                base_pwm = cruise;                    // โหมดพิเศษ: ความเร็วคงที่
            }
                ...
                // โหมดตรวจจับเซนเซอร์ ("fl", "fr" ฯลฯ)
                base_pwm = cruise;
            }

            dist += ((prevBase + base_pwm) / 2.0f) * SPEED_FACTOR_CM_S * dt;
            prevBase = base_pwm;
            ...
            if (isDigit(m[0])) {
                if (dist >= distance) should_stop = true;
            }
...
_line:
    ...
    if (nfc == 'n') {
        if (splr == 'p') {
            ...
            if (endt > 0) {
                Motor(-spl, -spr); delay(endt); Motor(-1, -1); delay(10);
            } else {
                    Motor(0, 0);
                    crossedLine = true;               
            }
        ...
        if (splr == 'p') {
            crossedLine = true;
            pidLoop(kp/2.5f, 1e-7f, 0.0125f, spl, spr, 80, stopC);
            if (endt > 0) { Motor(-spl,-spr); delay(endt); Motor(-1,-1); delay(10); }
            else Motor(0, 0);
        } else {
            if (distance > 0.0f) pidLoop(kp, 1e-7f, 0.0125f, slmotor, slmotor, 80, stopC);
            pidLoop(kp, 1e-7f, 0.0125f, slmotor, slmotor, 80, stopC);
        }
...
void bline(int spl, int spr, float kp, String target, char nfc, char splr, int power, String sensor, int endt)
{
    ...
    // =============== ถอยหลัง (ระยะทาง + โหมดตรวจจับเซนเซอร์) ===============
    {
        unsigned long t0 = millis(), prevT = t0;
        float pwm = minp, dist = 0.0f, prevBase = minp;
        bool accel = true, cruise_mode = false, decel = false;
...
_line:
    ...
    if (nfc == 'n') {
        if (splr == 'p') {
            ...
            if (endt > 0) {
                Motor(spl, spr); delay(endt); Motor(0, 0); delay(10);   // เบรก
            } else {
                    Motor(0, 0);
                    crossedLine = true;               
            }
