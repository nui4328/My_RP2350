...
// ==================== Sensor gyro ====================
uint16_t MyRP_Pico2::gyro(char z) {
    if(z == 'z')
        {
             return my.gyro('z') ;
        }
...
void MyRP_Pico2::bat_control() {
    ...
    // ป้องกัน scale เกินสำหรับ VNH7070ASTR
    scale = constrain(scale, 0.88f, 1.32f);
}
...
{
    // Battery Scale Update ทุก 70 ms
    ...
    pwmL = constrain((int)(pwmL * scale), 0, 3850);   // Safety limit สำคัญ
                       ...
                       char nfc, char splr, int power, String sensor, int endt)
{
    ...
    // === PID Parameters สำหรับ VNH7070ASTR ===
    float Ki       = 0.000037f;
    float Kd_min   = 0.019f;
    float Kd_max   = 0.020f;
    const float I_max = 460.0f;
    const float D_filter = 0.89f;
...

    {
        ...
        unsigned long lastBatteryUpdate = prevT;   // ใช้ตัวแปรจากคลาส
        ...
        while (true)
        {
            ...
            // Battery Update
            if (now - lastBatteryUpdate > 70) {
                updateBattery();
                float currentV = getBatteryVoltage();
                if (currentV >= VMIN && currentV <= VMAX && currentV > 0.5f) {
                    batteryScale = pow(VNOM / currentV, 0.95f);
                    batteryUsed = true;
                } else {
                    batteryScale = 1.0f;
                }
                batteryScale = constrain(batteryScale, 0.88f, 1.32f);
                lastBatteryUpdate = now;
            }

            // PID Calculation
            I_local += errors * dt;
            I_local = constrain(I_local, -I_max, I_max);
            ...
            float P = kp * errors;
            float I = Ki * I_local;
            float D_raw = (errors - prevE) / dt;
            float D = D_filter * prevD + (1.0f - D_filter) * current_Kd * D_raw;
            ...
            if (isSpecialMode) base_pwm = cruise;
            else if (isDigit(mode[0])) {
                if (skipAccel || endt == 0) base_pwm = cruise;
                else if (accel) {
                    pwm += ACCEL_RATE_PWM_S * dt;
                    if (pwm >= cruise) { pwm = cruise; accel = false; cruise_mode = true; }
                    base_pwm = pwm;
                }
                else if (cruise_mode) {
                    base_pwm = cruise;
                    if (dist >= distance * 0.65f) { cruise_mode = false; decel = true; decel_start = now; }
                }
                else if (decel) {
            ...
            else base_pwm = cruise;

            float compensated_pwm = base_pwm * batteryScale;
            compensated_pwm = constrain(compensated_pwm, minp, CRUISE_PWM_VAL * 1.20f);

            dist += ((prevBase + compensated_pwm) / 5.0f) * SPEED_FACTOR_CM_Sfw * dt;
            prevBase = compensated_pwm;
            ...
            // ... (ส่วน should_stop อื่น ๆ คงเดิมของคุณ) ...

            if (should_stop) break;

            Motor(constrain((int)(compensated_pwm - PID), -100, 100),
                  constrain((int)(compensated_pwm + PID), -100, 100));
...
_line_f:
    delay(10);
    if (splr != 'p') crossedLine = false;

    // ส่วน Lambda pidLoop (ปรับให้ใช้ batteryScale ถูกต้อง)
    auto pidLoop = [&](float kpu, float ki, float kd_base, int bl, int br, int dly, auto&& stopCond) {
        float localI = 0.0f;
        float localPrevE = 0.0f;

        unsigned long prevTimes = micros();
        float lastErrors = 0.0;
        ...
        while (true) {
            ...
            if (nowBat - lastBat > 80) {
                updateBattery();
                float v = getBatteryVoltage();
                if (v >= VMIN && v <= VMAX && v > 0.5f) {
                    batteryScale = pow(VNOM / v, 0.95f);
                }
                batteryScale = constrain(batteryScale, 0.88f, 1.32f);
                lastBat = nowBat;
            }
            ...
            /*
            localI += errors * ki;
            localI = constrain(localI, -I_max, I_max);
            ...
            */
            ...
            float dt = (currentTime - prevTimes) / 1000000.0f;
            if (dt < 0.005f) dt = 0.005f;
            prevTimes = currentTime;

            float errorss = errors / 2 ;

            float derivatives = (errorss - lastErrors)/dt;

            PID_output = (0.25 * errorss) + (0.015 * derivatives);

            lastErrors = errorss;

            int leftSpeeds  = constrain(slmotor - PID_output, -100, 100);
            int rightSpeeds = constrain(slmotor + PID_output, -100, 100);

            Motor(leftSpeeds, rightSpeeds);


            delayMicroseconds(60);   // เพิ่มเป็น 850 (สำคัญมากสำหรับลดชะลอ)
...
void MyRP_Pico2::fw_gyro(int spl, int spr, float kp, float distance_cm, int offset)
{
    ...
    while (true)
    {
        ...
        //Serial.print( " my.gyro('z')  " );   Serial.println( my.gyro('z'));
