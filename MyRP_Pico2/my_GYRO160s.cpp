...
// ค่าคงที่ที่ปรับให้ drift น้อยที่สุด (ทดลองจริงกับ BMI160)
const float my_GYRO160::GYRO_DEADZONE       = 0.25f;   // เพิ่มเพื่อตัด noise ดีขึ้น
const float my_GYRO160::ALPHA              = 0.95f;   // เชื่อ accel มากขึ้น (ลด drift)
const float my_GYRO160::ACCEL_FILTER_ALPHA = 0.20f;   // กรอง accel แรงขึ้น
const float my_GYRO160::BIAS_ALPHA         = 0.08f;   // ปรับ bias เร็วขึ้นมาก
