...
void setup() {
    ...
    trim_servo_0(8);    // arm_hand ซ้าย       ---> เพิ่มหรือลดตัวเลขในวงเล็บ  ค่าน้อยกางออก 6
    trim_servo_1(0);    // arm_shoulder  ซ้าย 
    trim_servo_2(0);    // arm_shoulder  ขวา   ค่าน้อยหุบเข้า
    trim_servo_3(15);   // arm_hand ขวา   ค่าน้อยกางออก 11
    trim_servo_4(-4);    // arm_can ซ้าย  ค่าน้อยไปข้างหน้า
    trim_servo_5(0);    // arm_can ขวา   ค่ามากไปข้างหน้า

    set_PULSES_updown(115);        // ตั้งค่า ewncoder ระยะขึ้นลงของแขน

    // set_PULSES_moveFW(50);     // ตั้งค่า ewncoder ระยะเดินหน้า
    // set_PULSES_moveBW(50);      // ตั้งค่า ewncoder ระยะถอยหลัง
    ...
    Set_distanceFW_to_turnGyro(70);  //---->> ตั้งค่าระยะเดินออกจากเส้นก่อนหมุนตัวหลังจากใช้ คำสั่งsetrobot_bw(2);
    Set_distanceBW_to_turnGyro(100);  //---->> ตั้งค่าระยะเดินออกจากเส้นก่อนหมุนตัวหลังจากใช้ คำสั่งsetrobot_fw(2);
    set_pid_turnL(3.4, 0.0012, 0.035);
    set_pid_turnR(3.4, 0.0012, 0.035);    

    //servo_begin();   //------>> เริ่มต้นมือจับ  มือคีบชนกัน   แขนชี้ไปข้างหน้า
    //set_servo_begin();    //------>> เริ่มต้นมือจับ  ตั้งค่าแขนวางมือลงช่องเก้บกระป๋อง
    //arm_can('R', 40);
    arm_ready(); 
    selectAndRunMode();
