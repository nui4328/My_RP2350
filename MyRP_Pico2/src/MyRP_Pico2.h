...
private:
    ...
    float batteryScale = 1.0f;           // สำคัญ! ตัวแปรนี้หายไป
    unsigned long lastBatteryUpdate = 0; // ใช้ควบคุมเวลา
    ...
    const float MIN_PWM_VAL       = 3.0f;
    const float ACCEL_RATE_PWM_S  = 50.0f;
    const float DECEL_RATE_PWM_S  = 50.0f;
...
public:
    ...
    float SPEED_FACTOR_CM_Sfw = 2.4f;
    float SPEED_FACTOR_CM_Sbw = 2.4f;

    // Distance Scale Functions
